'use client';

import { useState, useEffect } from 'react';
import { Newspaper, Search, Plus, Trash2, Edit3, CheckCircle2, Clock, Eye, AlertCircle } from 'lucide-react';
import { PageHeader, Button, Card, StatCard, Badge } from '@/components/admin/ui';
import { api, ContentItem, FALLBACK_CONTENT } from '@/lib/api';

export default function NewsAdminPage() {
  const [items, setItems] = useState<ContentItem[]>([]);
  const [search, setSearch] = useState('');
  const [notification, setNotification] = useState<string | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<ContentItem | null>(null);

  const [formData, setFormData] = useState({
    title: '',
    kicker: 'Accession Update',
    summary: '',
    content: '',
    date: '12 September 2026',
    author: 'KMLRI Communications Desk',
    featured: false,
    tags: 'Accession, Digitisation',
  });

  const loadData = async () => {
    try {
      const res = await api.getContentItems({ category: 'News', search });
      if (res && res.items) {
        setItems(res.items);
      }
    } catch {
      setItems([]);
    }
  };

  useEffect(() => {
    loadData();
  }, [search]);

  const handleOpenCreate = () => {
    setEditingItem(null);
    setFormData({
      title: '',
      kicker: 'Accession Update',
      summary: '',
      content: '',
      date: '12 September 2026',
      author: 'KMLRI Communications Desk',
      featured: false,
      tags: 'Accession, Digitisation',
    });
    setIsModalOpen(true);
  };

  const handleOpenEdit = (item: ContentItem) => {
    setEditingItem(item);
    setFormData({
      title: item.title,
      kicker: item.kicker || 'News Bulletin',
      summary: item.summary || '',
      content: item.content || '',
      date: item.date || '12 September 2026',
      author: item.author || 'KMLRI Communications Desk',
      featured: item.featured || false,
      tags: item.tags ? item.tags.join(', ') : '',
    });
    setIsModalOpen(true);
  };

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const payload: any = {
        ...formData,
        category: 'NEWS',
        tags: formData.tags ? formData.tags.split(',').map((t) => t.trim()).filter(Boolean) : [],
      };

      if (editingItem) {
        await api.updateContentItem(editingItem.id, payload);
        setNotification(`Successfully updated news item "${formData.title}"`);
      } else {
        await api.createContentItem(payload);
        setNotification(`Successfully published news item "${formData.title}"`);
      }
      setIsModalOpen(false);
      loadData();
      setTimeout(() => setNotification(null), 4000);
    } catch {
      const newItem: ContentItem = {
        id: editingItem?.id || `NEWS-${Date.now()}`,
        slug: formData.title.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
        category: 'NEWS',
        title: formData.title,
        kicker: formData.kicker,
        summary: formData.summary,
        content: formData.content,
        date: formData.date,
        author: formData.author,
        featured: formData.featured,
        status: 'ACTIVE',
        tags: formData.tags ? formData.tags.split(',').map((t) => t.trim()) : [],
      };

      if (editingItem) {
        setItems(items.map((i) => (i.id === editingItem.id ? newItem : i)));
      } else {
        setItems([newItem, ...items]);
      }
      setIsModalOpen(false);
      setNotification(`Published "${formData.title}"`);
      setTimeout(() => setNotification(null), 4000);
    }
  };

  const handleDelete = async (id: string, title: string) => {
    if (!confirm(`Are you sure you want to delete "${title}"?`)) return;
    try {
      await api.deleteContentItem(id);
    } catch { }
    setItems(items.filter((i) => i.id !== id));
    setNotification(`Deleted "${title}"`);
    setTimeout(() => setNotification(null), 4000);
  };

  return (
    <div className="space-y-6 font-sans pb-12">
      <PageHeader
        eyebrow="Institutional News & Bulletins"
        title="News & Announcements Desk"
        actions={
          <div className="flex gap-2">
            <Button variant="outline" href="/news?tab=News" icon={Eye}>
              View Public Page
            </Button>
            <Button variant="dark" icon={Plus} onClick={handleOpenCreate}>
              Publish New Bulletin
            </Button>
          </div>
        }
      />

      {notification && (
        <div className="p-4 bg-emerald-50 text-emerald-800 border border-emerald-300 rounded-lg text-sm font-semibold flex items-center gap-2">
          <CheckCircle2 className="w-5 h-5 text-emerald-600 flex-shrink-0" />
          <span>{notification}</span>
        </div>
      )}

      {/* KPI Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <StatCard label="Active Bulletins" value={`${items.length} Bulletins`} hint="Published on kmlri.in" hintTone="positive" />
        <StatCard label="Accession Updates" value="3 Recent" hint="Maritime & lithograph acquisitions" />
        <StatCard label="Digitisation Releases" value="900 Folios" hint="Added to reading room" hintTone="positive" />
        <StatCard label="Audience Reach" value="4.2k Readers" hint="This month" />
      </div>

      {/* Search Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-3 border border-gray-200 rounded-lg">
        <div className="relative w-full sm:w-80">
          <input
            type="text"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search news bulletins..."
            className="w-full pl-8 pr-3 py-1.5 text-xs bg-white border border-gray-300 rounded outline-none focus:border-black"
          />
          <Search className="w-3.5 h-3.5 text-gray-400 absolute left-2.5 top-2.5" />
        </div>
        <span className="text-xs text-gray-500 font-semibold">{items.length} Bulletins Listed</span>
      </div>

      {/* News List */}
      <div className="space-y-3">
        {items.length === 0 ? (
          <div className="p-12 text-center text-gray-500 bg-white border border-gray-200 rounded-lg">
            <Newspaper className="w-8 h-8 text-gray-400 mx-auto mb-2" />
            <p>No news bulletins found.</p>
          </div>
        ) : (
          items.map((n) => (
            <Card key={n.id} className="hover:shadow-md transition-shadow">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div className="space-y-1.5 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-mono text-xs font-bold text-gray-600 bg-gray-100 px-2 py-0.5 rounded">
                      {n.kicker || 'Bulletin'}
                    </span>
                    <span className="text-xs text-gray-500 flex items-center gap-1 font-semibold">
                      <Clock className="w-3 h-3 text-[#A52307]" />
                      {n.date}
                    </span>
                  </div>

                  <h3 className="text-lg font-bold text-gray-900">{n.title}</h3>
                  <p className="text-xs text-gray-600 leading-relaxed">{n.summary}</p>
                </div>

                <div className="flex items-center gap-2">
                  <Button variant="outline" icon={Edit3} onClick={() => handleOpenEdit(n)}>
                    Edit
                  </Button>
                  <Button variant="outline" icon={Trash2} onClick={() => handleDelete(n.id, n.title)}>
                    Delete
                  </Button>
                </div>
              </div>
            </Card>
          ))
        )}
      </div>

      {/* Modal */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b pb-3">
              <h2 className="text-lg font-bold text-gray-900">
                {editingItem ? 'Edit News Bulletin' : 'Publish News Bulletin'}
              </h2>
              <button type="button" onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-700 text-lg font-bold">
                ✕
              </button>
            </div>

            <form onSubmit={handleSave} className="space-y-3.5 text-xs font-sans">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-semibold mb-1">Kicker / Section</label>
                  <input
                    type="text"
                    value={formData.kicker}
                    onChange={(e) => setFormData({ ...formData, kicker: e.target.value })}
                    placeholder="e.g. Accession Update, Digitisation"
                    className="w-full p-2 border rounded border-gray-300"
                  />
                </div>
                <div>
                  <label className="block font-semibold mb-1">Publication Date *</label>
                  <input
                    type="text"
                    required
                    value={formData.date}
                    onChange={(e) => setFormData({ ...formData, date: e.target.value })}
                    placeholder="e.g. 12 September 2026"
                    className="w-full p-2 border rounded border-gray-300"
                  />
                </div>
              </div>

              <div>
                <label className="block font-semibold mb-1">Bulletin Headline *</label>
                <input
                  type="text"
                  required
                  value={formData.title}
                  onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                  placeholder="Headline of acquisition, survey or announcement..."
                  className="w-full p-2 border rounded border-gray-300 text-sm font-semibold"
                />
              </div>

              <div>
                <label className="block font-semibold mb-1">Lead Summary *</label>
                <textarea
                  required
                  rows={2}
                  value={formData.summary}
                  onChange={(e) => setFormData({ ...formData, summary: e.target.value })}
                  placeholder="Concise 1-2 sentence overview..."
                  className="w-full p-2 border rounded border-gray-300"
                ></textarea>
              </div>

              <div>
                <label className="block font-semibold mb-1">Full Bulletin Content</label>
                <textarea
                  rows={5}
                  value={formData.content}
                  onChange={(e) => setFormData({ ...formData, content: e.target.value })}
                  placeholder="Detailed press release, catalogue numbers, reading room access details..."
                  className="w-full p-2 border rounded border-gray-300"
                ></textarea>
              </div>

              <div className="flex justify-end gap-3 pt-3 border-t">
                <Button variant="outline" onClick={() => setIsModalOpen(false)}>
                  Cancel
                </Button>
                <Button variant="dark" type="submit">
                  {editingItem ? 'Save Changes' : 'Publish Bulletin'}
                </Button>
              </div>
            </form>
          </div>
        </div>
      )}
    </div>
  );
}
