...
export default function VendorsAdminPage() {
  ...
  const handleDeactivate = async (v: Vendor) => {
    if (!(await confirmDialog({ message: `Deactivate vendor "${v.name}"?`, variant: 'danger' }))) return;
    try {
  ...
  const handleDelete = async (v: Vendor) => {
    if (!(await confirmDialog({ message: `Permanently remove vendor "${v.name}"? This cannot be undone.`, variant: 'danger' }))) return;
    try {
